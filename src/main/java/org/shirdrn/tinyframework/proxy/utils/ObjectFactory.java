...
public class ObjectFactory {
	...
	private static ClassLoader DEFAULT_CLASSLOADER = ObjectFactory.class.getClassLoader();
	...
	public static Object getInstance(String className) {
		...
		try {
			Class<?> clazz = Class.forName(className, true, getClassLoader(DEFAULT_CLASSLOADER));
	...
	/**
	 * Instantiate a {@link Class} instance for given {@linkplain className}
	 * {@linkplain baseClass}, {@linkplain classLoader} and several {@linkplain parameters}.
	 * @param className
	 * @param classLoader
	 * @return
	 */
	 ...
	 * {@linkplain classLoader} is this {@link ObjectFactory}'s class loader..
	 * @param className
	 * @param parameters
	 * @return
	 */
	...
	/**
	 * Instantiate a {@link Class} instance for given parameters:
	 * {@linkplain className}, {@linkplain parameters} and {@linkplain classLoader}.
	 * @param className
	 * @param classLoader
	 * @param parameters
	 * @return
	 ...
	 * and {@linkplain baseClass}, default {@linkplain classLoader} is this
	 * {@link ObjectFactory}'s class loader.
	 * @param className
	 * @return
	 */
	@SuppressWarnings("unchecked")
	public static <T> Class<T> getClass(String className, Class<T> baseClass) {
		Class<T> clazz = null;
		try {
			clazz = (Class<T>) Class.forName(className, true, getClassLoader(DEFAULT_CLASSLOADER));
		} catch (Exception e) {
			e.printStackTrace();
		}
		return clazz;
